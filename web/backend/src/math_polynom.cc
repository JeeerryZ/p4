...
double eval_term1(P4POLYNOM1 p, double t)
{
    ...
    while (p != nullptr) {
        if (p->exp != 0)
            s += p->coeff * std::pow(t, (double)(p->exp));
        else
            s += p->coeff;
        p = p->next_term1;
    }
...
double eval_term2(P4POLYNOM2 f, double *value)
{
    ...
    while (f != nullptr) {
        if (f->exp_x != 0 && f->exp_y != 0)
            s += f->coeff * std::pow(value[0], (double)(f->exp_x))
                          * std::pow(value[1], (double)(f->exp_y));
        else if (f->exp_x != 0)
            s += f->coeff * std::pow(value[0], (double)(f->exp_x));
        else if (f->exp_y != 0)
            s += f->coeff * std::pow(value[1], (double)(f->exp_y));
        else
            s += f->coeff;
        f = f->next_term2;
    }
...
//                              EVAL_TERM3
// -----------------------------------------------------------------------
// value[0]=r, value[1]=theta
double eval_term3(P4POLYNOM3 F, double *value)
{
    double s = 0.0;
    double Co = std::cos(value[1]);
    double Si = std::sin(value[1]);

    while (F != nullptr) {
        double t = 1.0;
        if (F->exp_r != 0)
            t *= std::pow(value[0], (double)(F->exp_r));
        if (F->exp_Co != 0)
            t *= std::pow(Co, (double)(F->exp_Co));
        if (F->exp_Si != 0)
            t *= std::pow(Si, (double)(F->exp_Si));
        s += t * F->coeff;
        F = F->next_term3;
    }
    return s;
...
// -----------------------------------------------------------------------
//                   PRINTTERM2  (used in Maple script preparation)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
//                   PRINTTERM3  (used in Maple script preparation)
// -----------------------------------------------------------------------
