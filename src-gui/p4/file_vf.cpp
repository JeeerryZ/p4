...
/*void QInputVF::prepareReduceParameters(QTextStream *fp)
{
    ...
    s.asprintf("numeric:=%d$\n", numeric_);
    *fp << s;
    s.asprintf("user_precision:=%d$\n", precision);
    *fp << s;
    *fp << "epsilon:=" << epsilon_ << "$\n";
    s.asprintf("test_sep:=%d$\n", _testsep);
    *fp << s;
    s.asprintf("taylor_level:=%d$\n", taylorlevel_);
    *fp << s;
    s.asprintf("numeric_level:=%d$\n", numericlevel_);
    *fp << s;
    s.asprintf("max_level:=%d$\n", maxlevel_);
    *fp << s;
    s.asprintf("weakness_level:=%d$\n", weakness);
    *fp << s;
    ...
    if (typeofstudy_ == TYPEOFSTUDY_ONE) {
        ...
        *fp << "x0:=" << x0_ << "$\n";
        *fp << "y0:=" << y0_ << "$\n";
        s.asprintf("x_min:=x0+(%f)$\n", (float)(X_MIN));
        *fp << s;
        s.asprintf("x_max:=x0+(%f)$\n", (float)(X_MAX));
        *fp << s;
        s.asprintf("y_min:=y0+(%f)$\n", (float)(Y_MIN));
        *fp << s;
        s.asprintf("y_max:=y0+(%f)$\n", (float)(Y_MAX));
        *fp << s;
    } else {
        s.asprintf("p:=%d$\n", p);
        *fp << s;
...
void QInputVF::prepareMapleParameters(QTextStream *fp)
{
    ...
    *fp << "test_sep := " << booleanString(testsep_) << ":\n";
    Q_ASSERT(precision_ > 0);
    *fp << QString("user_precision := %1:\n").arg(precision_);
    *fp << QString("user_precision0 := %1:\n").arg(precision0_);
    *fp << QString("taylor_level := %1:\n").arg(taylorlevel_);
    *fp << QString("numeric_level := %1:\n").arg(numericlevel_);
    *fp << QString("max_level := %1:\n").arg(maxlevel_);
    *fp << QString("weakness_level := %1:\n").arg(weakness_);
    ...
    if (typeofstudy_ == TYPEOFSTUDY_ONE) {
        ...
        *fp << QString("x0 := %1:\n").arg(x0_);
        *fp << QString("y0 := %1:\n").arg(y0_);
        *fp << QString("x_min := x0+(%1):\n").arg((float)(X_MIN));
        *fp << QString("x_max := x0+(%1):\n").arg((float)(X_MAX));
        *fp << QString("y_min := y0+(%1):\n").arg((float)(Y_MIN));
        *fp << QString("y_max := y0+(%1):\n").arg((float)(Y_MAX));
    } else {
        *fp << QString("user_p := %1:\n").arg(p_);
        *fp << QString("user_q := %1:\n").arg(q_);
    }
...
void QInputVF::finishEvaluation(int exitCode)
{
    ...
    if (evalProcess_ != nullptr) {
        ...
        if (evalProcess_->state() == QProcess::Running) {
            ...
            if (!processfailed_)
                buf += QString("The process finished normally (%1)\n")
                           .arg(evalProcess_->exitCode());
            else {
                buf += QString("The process stopped abnormally (%1 : ")
                           .arg(evalProcess_->exitCode());
                buf += processError_;
                buf += ")\n";
            }
...
void QInputVF::onTerminateButton(void)
{
    if (evalProcess_ != nullptr) {
        if (evalProcess_->state() == QProcess::Running) {
            ...
            emit logOutput("Kill signal sent to process.\n");
            processfailed_ = true;
